...
|--------------------------------------------------------------------------
*/
namespace App\Database; /** ประกาศ namespace เพื่อไว้ใช้งานในไฟล์ index โดยเรียกผ่าน autoload */
use PDO;
use PDOException;
class DB {
    private static $connect; /** กำหนด Property $connect */
    private static $instance = null; /** กำหนด Property $instance มีค่าเริ่มต้นเป็น null */
    private static $response = true; /** กำหนด Property $response มีค่าเริ่มต้นเป็น true */

    private function __construct() {
        self::$connect = null; /** กำหนดค่าเริ่มต้นเป็น null, เวลาเข้าถึงข้อมูล static ต้องใช้ self:: ในการเข้าถึง */
        try {
            /**
             * สร้าง instance PDO ค่า $_ENV ได้มาจากไฟล์ .env 
             */
            self::$connect = new PDO('mysql:host='.$_ENV['DB_HOST'].'; 
